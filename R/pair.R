#' This function transforms data that are given in an arm-based format (e.g. input format for WinBUGS)
#' to a contrast-based format that is needed as input to R function netmeta. The function can transform
#' data with binary, continuous, or generic outcomes as well as incidence rates from arm-based to contrast-based
#' format.
#'\code{pair} implements .....
#' @usage pair(treat, event, n, mean, sd, TE, seTE, time, data = NULL, studlab, ...)
#' @param treat	A list or vector with treatment information for individual treatment arms (see Details).
#' @param event	A list or vector with information on number of events for individual treatment arms (see Details).
#' @param n	A list or vector with information on number of observations for individual treatment arms (see Details).
#' @param mean	A list or vector with estimated means for individual treatment arms (see Details).
#' @param sd	A list or vector with information on the standard deviation for individual treatment arms (see Details).
#' @param TE	A list or vector with estimated treatment effects for individual treatment arms (see Details).
#' @param seTE A list or vector with standard errors of estimated treatment effect for individual treatment arms (see Details).
    ...
    for (i in 1:(narms - 1)) {
      ...
      for (j in (i + 1):narms) {
        ...
        dat <- data.frame(TE = NA, seTE = NA, studlab = studlab,
                          treat1 = treat[[i]], treat2 = treat[[j]], event1 = event[[i]],
                          n1 = n[[i]], event2 = event[[j]], n2 = n[[j]])
  ...
  if (type == "continuous") {
    ...
    for (i in 1:(narms - 1)) {
      ...
      for (j in (i + 1):narms) {
        ...
        dat <- data.frame(TE = NA, seTE = NA, studlab = studlab,
                          treat1 = treat[[i]], treat2 = treat[[j]], n1 = n[[i]],
                          mean1 = mean[[i]], sd1 = sd[[i]], n2 = n[[j]],
                          mean2 = mean[[j]], sd2 = sd[[j]])
  ...
  if (type == "generic") {
    ...
    for (i in 1:(narms - 1)) {
      ...
      for (j in (i + 1):narms) {
        ...
        dat <- data.frame(TE = NA, seTE = NA, studlab = studlab,
                          treat1 = treat[[i]], treat2 = treat[[j]], TE1 = TE[[i]],
                          seTE1 = seTE[[i]], TE2 = TE[[j]], seTE2 = seTE[[j]])
  ...
  if (type == "count") {
    ...
    for (i in 1:(narms - 1)) {
      ...
      for (j in (i + 1):narms) {
        ...
        dat <- data.frame(TE = NA, seTE = NA, studlab = studlab,
                          treat1 = treat[[i]], treat2 = treat[[j]], event1 = event[[i]],
                          time1 = time[[i]], event2 = event[[j]], time2 = time[[j]])
